    ...
    if (empty($name)||empty($lastname)||empty($email)||empty($pass)||empty($rePass)){
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)){
            ...
            if ($resultControl > 0){
                ...
                $sql = "INSERT INTO users (first_name,last_name,email,gender,national_insurence_number,birth_date,birth_place,streetName,houseNumber,town,passwordPatient) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
