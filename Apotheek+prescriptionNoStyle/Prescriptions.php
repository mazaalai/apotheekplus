        ...
        if (isset($_SESSION['idDoctor'])){
            ...
            // output data of each row
                while($row = $result->fetch_assoc()) {
                    ...
                    {
                        echo ("Last name: " . $row["lastName"]." - First name: " . $row["firstName"]." - medicine: " . $row["medicine"]. " - urgency: " . $row["urgency"]. " - reason: " . $row["reason"]. "<br>");
                    }
                }
                echo ('<br>');
                $result = $conn->query($sql);
            ...
            {
                header("Location: Prescriptions.php?error=failed");
                exit(); 
            }
        }
        else if (isset($_SESSION['userId'])){
            echo('<center><!--<h1> Vraag hier een medicijn aan </h1> -->
                ...
                Urgency<br> 
                <input type="radio" name="urgency" value="Urgent">very urgent
                <input type="radio" name="urgency" value="Not Urgent">not urgent <br><br>
                <input type="submit" name="submit" value="submit">
                </form></center>');
        }
        else{
            echo ('<center><a href="login.php" class="">Login</a></center>');
        }
        ?>

        <div class="names">
            <form name="form2" action="" method="post">
                <p class="small">Concept by: Alex Willemen - Taivan Enkhbayar - Tim Philippe - Jochen Snoeks - Luca De Boeck - Serhat</p>
