<html>
    <head>
        <title>Login</title>
        <meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1">
        <link rel="stylesheet" type = "text/css" href="stylesheet.css"> </link>
    </head>

    <body>
        <div class="navbar">
                <a class="bold" href="homepage.php">Apotheek+</a>
                <div class="dropdown">
                    <button class="dropbtn">☰
                    <i class="fa fa-caret-down"></i>
                    </button>
                    <div class="dropdown-content">
                    <a href="MedicalDocuments.php">Medical Documents</a>
                    <a href="Prescriptions.php">Prescriptions</a>
                    <a href="AskYourDoctor.php">Ask your doctor</a>
                    </div>
                </div> 
                <a href="login.php" class="testright">Account</a>
                </div>

        <div class="logincontainer">
            <form name="form1" action="" method="post">
                <img src = "img/user.png" class = "user">
                <h2 class="login"> Are you a client of a doctor? </h2>
                <a class="choose" href="registration.php">I am a client<br><br><br></a>
                <a class="choose" href="registrationdoctor.php">I am a doctor</a>
            </form>
            </div>

    </body>
</html>

